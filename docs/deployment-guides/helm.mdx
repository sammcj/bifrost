...
```bash
helm install bifrost bifrost/bifrost --set image.tag=v1.3.36
```
...
# production.yaml
image:
  tag: "v1.3.36"  # Required: specify the Bifrost version
...
|-----------|-------------|---------|
| `image.tag` | **Required.** Bifrost image version (e.g., v1.3.36) | `""` |
