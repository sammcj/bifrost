---
title: "Adding a backend for vector store"
...
*   **Interfaces**: The `VectorStore` interface defines the contract for all vector operations.
*   **Database-Specific Implementations**: Each vector database backend implements the interface independently, using native clients and optimizing for that database's strengths.
*   **Extensibility**: Adding a new database requires implementing the interface with the database-specific client, rather than forcing a common abstraction layer.
