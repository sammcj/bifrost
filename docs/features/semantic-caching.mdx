...
**Location**: `response.ExtraFields.RawResponse` (as a map)

**Variables**:
- `bifrost_cached` (boolean): `true` when response served from cache
- `bifrost_cache_key` (string): Unique cache entry ID for management operations  
- `bifrost_cache_type` (string): `"semantic"` for similarity match, `"direct"` for hash match

**Semantic Cache Only**:
- `bifrost_cache_threshold` (number): Similarity threshold used for the match
- `bifrost_cache_similarity` (number): Similarity score for the match
