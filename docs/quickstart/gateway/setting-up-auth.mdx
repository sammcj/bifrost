...
When authentication is enabled, API calls and inference calls can use:
...
### Example: Using Bearer Auth for Inference call
```bash
# First, encode your credentials as base64
# echo -n "your-username:your-password" | base64

# Using curl with Bearer Token
curl -X POST http://localhost:8080/v1/chat/completions \
  -H "Authorization: Bearer $(echo -n 'your-username:your-password' | base64)" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "openai/gpt-4o",
    "messages": [{"role": "user", "content": "Hello!"}]
